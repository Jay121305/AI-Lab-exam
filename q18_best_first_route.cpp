// Q18. Interactive Best First Search for route finding.
// It expands the city estimated closest to the goal; this is fast but not always optimal.
#include <bits/stdc++.h>
using namespace std;
...
int main() {
    ...
    priority_queue<Node, vector<Node>, greater<Node>> pq; set<string> seen;
    pq.push({h(start, goal), 0, start, {start}});
    while (!pq.empty()) {
        auto [est, cost, city, path] = pq.top(); pq.pop();
        if (seen.count(city)) continue; seen.insert(city);
        if (city == goal) { cout << "Route: "; for (string c : path) cout << c << " "; cout << "\nCost: " << cost << "\n"; return 0; }
        for (auto [n, d] : graph[city]) { auto p = path; p.push_back(n); pq.push({h(n, goal), cost + d, n, p}); }
    }
}
